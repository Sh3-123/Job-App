...
(function () {
  ...
  const JOB_STATUS_STORAGE_KEY = "jobTrackerStatus";
  const STATUS_HISTORY_STORAGE_KEY = "jobTrackerStatusHistory";
  const TEST_CHECKLIST_STORAGE_KEY = "jobTrackerTestChecklist";
  const PROOF_ARTIFACTS_STORAGE_KEY = "jobTrackerProofArtifacts";
  ...
  function getJobStatus(jobId) {
    try {
      const raw = localStorage.getItem(JOB_STATUS_STORAGE_KEY);
  ...
  function addStatusHistory(jobId, status) {
    try {
      ...
      if (job) {
        ...
        localStorage.setItem(STATUS_HISTORY_STORAGE_KEY, JSON.stringify(trimmed));
      }
    } catch (_) { }
  }
  ...
  function setTestChecklist(checklist) {
    try {
      localStorage.setItem(TEST_CHECKLIST_STORAGE_KEY, JSON.stringify(checklist));
    } catch (_) { }
  }

  function toggleTestItem(itemKey) {
    const checklist = getTestChecklist();
    checklist[itemKey] = !checklist[itemKey];
  ...
  function areAllTestsPassed() {
    ...
    return requiredTests.every(function (test) { return checklist[test] === true; });
  }

  // Proof artifacts management
  function getProofArtifacts() {
    try {
      const raw = localStorage.getItem(PROOF_ARTIFACTS_STORAGE_KEY);
  ...
  function setProofArtifacts(artifacts) {
    try {
      localStorage.setItem(PROOF_ARTIFACTS_STORAGE_KEY, JSON.stringify(artifacts));
    } catch (_) { }
  }
  ...
  function showToast(message) {
    ...
    var existingToast = document.getElementById("kn-toast");
  ...
  function filterAndSortJobs(jobs, filters, prefs) {
    ...
    if (filters.status) {
      pairs = pairs.filter(function (p) { return getJobStatus(p.job.id) === filters.status; });
    }
    if (filters.location) {
  ...
  function renderJobCard(job, opts) {
    ...
    if (currentStatus === "Applied") statusBadgeClass = "kn-badge--status-applied";
    else if (currentStatus === "Rejected") statusBadgeClass = "kn-badge--status-rejected";
    else if (currentStatus === "Selected") statusBadgeClass = "kn-badge--status-selected";
    else statusBadgeClass = "kn-badge--status-not-applied";
    ...
    return (
      ...
      "</div>" +
      "<div class=\"kn-job-card__status-section\">" +
      "<span class=\"kn-job-card__status-label\">Status:</span>" +
      "<span class=\"kn-badge kn-badge--status " + statusBadgeClass + "\">" + currentStatus + "</span>" +
      "<div class=\"kn-status-buttons\" data-job-id=\"" + (job.id || "") + "\">" +
      "<button type=\"button\" class=\"kn-status-btn " + (currentStatus === "Not Applied" ? "kn-status-btn--active" : "") + "\" data-status=\"Not Applied\">Not Applied</button>" +
      "<button type=\"button\" class=\"kn-status-btn " + (currentStatus === "Applied" ? "kn-status-btn--active" : "") + "\" data-status=\"Applied\">Applied</button>" +
      "<button type=\"button\" class=\"kn-status-btn " + (currentStatus === "Rejected" ? "kn-status-btn--active" : "") + "\" data-status=\"Rejected\">Rejected</button>" +
      "<button type=\"button\" class=\"kn-status-btn " + (currentStatus === "Selected" ? "kn-status-btn--active" : "") + "\" data-status=\"Selected\">Selected</button>" +
      "</div>" +
      "</div>" +
  ...
  const routes = {
    ...
    "/jt/07-test": { key: "test", title: "Test Checklist – Job Notification Tracker" },
    "/jt/08-ship": { key: "ship", title: "Ship – Job Notification Tracker" },
  };
  ...
  function renderDashboard(container) {
    ...
    container.innerHTML =
      ...
      "<select id=\"kn-filter-status\" class=\"kn-input\" aria-label=\"Status\">" +
      ...
      "</select>" +
      "</div>" +
      "<button type=\"button\" class=\"kn-button kn-button--secondary kn-button--compact\" id=\"kn-clear-filters\" style=\"margin-left: auto;\">Clear All Filters</button>" +
      "</div>" +
      "</div>" +
    ...
    attachClearFiltersButton(container);
  ...
  function getDashboardFilters(container) {
    ...
    return {
      ...
      status: (root.querySelector("#kn-filter-status") || {}).value,
  ...
  function attachDashboardFilters(container) {
    var root = container.querySelector(".kn-route--dashboard") || container;
    var inputs = root.querySelectorAll("#kn-filter-keyword, #kn-filter-status, #kn-filter-location, #kn-filter-mode, #kn-filter-experience, #kn-filter-source, #kn-filter-sort, #kn-filter-show-matches");
  ...
  function attachClearFiltersButton(container) {
    var clearBtn = container.querySelector("#kn-clear-filters");
    if (!clearBtn) return;
    ...
    clearBtn.addEventListener("click", function () {
      ...
      var keywordInput = root.querySelector("#kn-filter-keyword");
      var statusSelect = root.querySelector("#kn-filter-status");
      var locationSelect = root.querySelector("#kn-filter-location");
      var modeSelect = root.querySelector("#kn-filter-mode");
      var experienceSelect = root.querySelector("#kn-filter-experience");
      var sourceSelect = root.querySelector("#kn-filter-source");
      var sortSelect = root.querySelector("#kn-filter-sort");
      var matchCheckbox = root.querySelector("#kn-filter-show-matches");
      ...
      // Trigger update to refresh the job list
      updateDashboardJobList(container);
    });
  }
  ...
  function attachDashboardCardActions(container) {
    ...
    listEl.addEventListener("click", function (e) {
      ...
      var statusBtn = e.target.closest(".kn-status-btn");
      if (statusBtn) {
        e.preventDefault();
        var statusButtonsContainer = statusBtn.closest(".kn-status-buttons");
        var jobId = statusButtonsContainer ? statusButtonsContainer.getAttribute("data-job-id") : null;
  ...
  `;
    ...
    if (statusHistory.length > 0) {
      const statusUpdatesHtml = `
        <div class="digest-status-updates" style="margin-top: var(--kn-space-4);">
          <h2 class="kn-heading-3">Recent Status Updates</h2>
          <div class="status-updates-list">
            ${statusHistory.slice(0, 10).map(function (item) {
        ...
        let statusClass = "";
        if (item.status === "Applied") statusClass = "kn-badge--status-applied";
        else if (item.status === "Rejected") statusClass = "kn-badge--status-rejected";
        else if (item.status === "Selected") statusClass = "kn-badge--status-selected";
        else statusClass = "kn-badge--status-not-applied";

        return `
                <div class="status-update-item">
                  <div class="status-update-header">
                    <h4 class="status-update-title">${item.jobTitle}</h4>
                    <span class="kn-badge kn-badge--status ${statusClass}">${item.status}</span>
                  </div>
                  <div class="status-update-meta">
                    <span class="status-update-company">${item.company}</span>
                    <span class="status-update-separator">•</span>
      ...
      digestContainer.insertAdjacentHTML('beforeend', statusUpdatesHtml);
    }
  }
  ...
  function renderProof(container) {
    ...
    // Define the 8 steps
    const steps = [
      { label: 'User Preferences Setup', completed: !!getPreferences() },
      { label: 'Match Score Algorithm', completed: true }, // Always true since logic exists
      { label: 'Save & Apply Functionality', completed: getSavedIds().length > 0 },
      { label: 'Status Tracking System', completed: true }, // Always true since logic exists
      { label: 'Daily Digest Engine', completed: true }, // Always true since logic exists
      { label: 'Test Checklist System', completed: allTestsPassed },
      { label: 'Artifact Collection', completed: allLinksProvided },
      { label: 'Final Deployment', completed: shipped }
    ];
    ...
    container.innerHTML = `
      <header class="kn-route__header">
        <h1 class="kn-heading-1">Proof & Submission</h1>
        <p class="kn-route__subtext">
      ...
      <section class="kn-proof">
        ${shipped ? `
          <div class="kn-alert kn-alert--success kn-shipped-message">
            <div class="kn-shipped-content">
              <div class="kn-shipped-icon">✓</div>
              <div>
                <h2 class="kn-alert__title">Project 1 Shipped Successfully.</h2>
                <p class="kn-alert__body">All requirements met. Ready for evaluation.</p>
              </div>
            </div>
          </div>
        ` : ''}

        <div class="kn-card kn-card--proof">
          <div class="kn-card__header">
            <h2 class="kn-heading-3">A) Step Completion Summary</h2>
            <p class="kn-card__subtitle">
              Track progress across all implementation phases.
            </p>
          </div>
          <div class="kn-card__body">
            <div class="kn-step-progress">
              <div class="kn-step-progress__header">
                <span class="kn-step-progress__label">Overall Progress:</span>
                <span class="kn-step-progress__value">${completedCount} / ${totalSteps} Steps</span>
              </div>
              <div class="kn-step-progress__bar">
                <div class="kn-step-progress__fill" style="width: ${(completedCount / totalSteps) * 100}%"></div>
              </div>
            </div>

            <div class="kn-step-list">
              ${steps.map((step, idx) => `
                <div class="kn-step-item ${step.completed ? 'kn-step-item--completed' : 'kn-step-item--pending'}">
                  <div class="kn-step-item__icon">
                    ${step.completed ? '✓' : `${idx + 1}`}
                  </div>
                  <div class="kn-step-item__content">
                    <span class="kn-step-item__label">${step.label}</span>
                    <span class="kn-step-item__status">${step.completed ? 'Completed' : 'Pending'}</span>
                  </div>
                </div>
              `).join('')}
            </div>
        ...
        <div class="kn-card kn-card--proof">
          <div class="kn-card__header">
            <h2 class="kn-heading-3">B) Artifact Collection</h2>
            <p class="kn-card__subtitle">
              Provide links to prove project completion. All fields are required.
            </p>
          </div>
          <div class="kn-card__body kn-card__body--stacked">
            <div class="kn-field">
              <label class="kn-field__label" for="lovable-link">
              ...
              <input
                ...
                data-artifact-field="lovableLink"
              />
              <p class="kn-field__hint">
                ${validateUrl(artifacts.lovableLink) ? '✓ Valid URL provided' : 'Enter your Lovable project URL'}
              </p>
            </div>

            <div class="kn-field">
              <label class="kn-field__label" for="github-link">
              ...
              <input
                ...
                data-artifact-field="githubLink"
              />
              <p class="kn-field__hint">
                ${validateUrl(artifacts.githubLink) ? '✓ Valid URL provided' : 'Enter your GitHub repository URL'}
              </p>
            </div>

            <div class="kn-field">
              <label class="kn-field__label" for="deployed-link">
                Live Deployment URL *
              </label>
              <input
                id="deployed-link"
                ...
                data-artifact-field="deployedLink"
              />
              <p class="kn-field__hint">
                ${validateUrl(artifacts.deployedLink) ? '✓ Valid URL provided' : 'Enter your deployed application URL (Vercel or equivalent)'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="kn-card kn-card--proof">
          <div class="kn-card__header">
            <h2 class="kn-heading-3">Final Submission</h2>
            <p class="kn-card__subtitle">
              Copy formatted submission text for evaluation.
            </p>
          </div>
          <div class="kn-card__body">
            ${!allTestsPassed || !allLinksProvided ? `
              <div class="kn-alert kn-alert--error">
                <h3 class="kn-alert__title">⚠ Submission Requirements Not Met</h3>
                <p class="kn-alert__body">
                  Before submitting, ensure:
                  <br>• All 10 test checklist items are passed
                  <br>• All 3 artifact links are provided and valid
                </p>
              </div>
            ` : ''}
            ...
            <button 
              type="button" 
              class="kn-button ${shipped ? 'kn-button--success' : 'kn-button--primary'}" 
              id="copy-submission-btn"
              ${!shipped ? 'disabled' : ''}
            >
              ${shipped ? '📋 Copy Final Submission' : '🔒 Complete All Requirements First'}
            </button>
          </div>
        </div>

        <div class="kn-proof-status">
          <div class="kn-proof-status__label">Project Status:</div>
          <span class="kn-badge kn-badge--status kn-badge--status-${shipped ? 'shipped' : (completedCount > 0 ? 'in-progress' : 'not-started')}">
            ${shipped ? 'Shipped' : (completedCount > 0 ? 'In Progress' : 'Not Started')}
          </span>
        </div>
      </section>
    `;
    ...
    const inputs = container.querySelectorAll('.kn-proof-input');
    inputs.forEach(function (input) {
      input.addEventListener('blur', function () {
        const field = this.getAttribute('data-artifact-field');
        const currentArtifacts = getProofArtifacts();
        currentArtifacts[field] = this.value.trim();
        setProofArtifacts(currentArtifacts);

        // Re-render to update validation and status
        renderProof(container);
      });
    });

    // Attach event listener for copy button
    const copyBtn = container.querySelector('#copy-submission-btn');
    if (copyBtn && shipped) {
      copyBtn.addEventListener('click', function () {
...
------------------------------------------`;
        ...
        // Copy to clipboard
        navigator.clipboard.writeText(submissionText).then(function () {
          showToast('✓ Submission text copied to clipboard');
        }).catch(function () {
  ...
  function renderTest(container) {
    ...
    const testItems = [
      {
        ...
        tooltip: 'Set preferences in Settings, refresh the page, and verify they are still present'
      },
      {
        key: 'match_score',
        label: 'Match score calculates correctly',
        tooltip: 'Check that job cards display match scores based on your preferences'
      },
      {
        key: 'show_matches_toggle',
        label: '"Show only matches" toggle works',
        tooltip: 'Toggle the checkbox on Dashboard and verify filtering behavior'
      },
      {
        key: 'save_persist',
        label: 'Save job persists after refresh',
        tooltip: 'Save a job, refresh the page, and verify it remains saved'
      },
      {
        key: 'apply_new_tab',
        label: 'Apply opens in new tab',
        tooltip: 'Click "Apply" button and verify it opens in a new tab'
      },
      {
        key: 'status_persist',
        label: 'Status update persists after refresh',
        tooltip: 'Change a job status, refresh the page, and verify the status is preserved'
      },
      {
        key: 'status_filter',
        label: 'Status filter works correctly',
        tooltip: 'Use the status dropdown filter on Dashboard and verify results'
      },
      {
        key: 'digest_generates',
        label: 'Digest generates top 10 by score',
        tooltip: 'Visit Digest page and verify it shows the top 10 jobs sorted by match score'
      },
      {
        key: 'digest_persists',
        label: 'Digest persists for the day',
        tooltip: 'Generate digest, refresh page, and verify the same digest is shown'
      },
      {
        key: 'no_console_errors',
        label: 'No console errors on main pages',
        tooltip: 'Open browser console and visit all main pages, checking for errors'
      }
    ];

    const passedCount = testItems.filter(function (item) { return checklist[item.key] === true; }).length;
    const totalCount = testItems.length;
    const allPassed = passedCount === totalCount;

    container.innerHTML = `
      <header class="kn-route__header">
        <h1 class="kn-heading-1">Test Checklist</h1>
        <p class="kn-route__subtext">
          Verify all critical functionality before shipping.
        </p>
      </header>
      
      <section class="kn-test-checklist">
        <div class="kn-test-summary">
          <div class="kn-test-summary__score">
            <span class="kn-test-summary__label">Tests Passed:</span>
            <span class="kn-test-summary__value">${passedCount} / ${totalCount}</span>
          </div>
          ${!allPassed ? `
            <div class="kn-test-summary__warning">
              <span class="kn-test-summary__warning-icon">⚠</span>
              <span>Resolve all issues before shipping.</span>
            </div>
          ` : `
            <div class="kn-test-summary__success">
              <span class="kn-test-summary__success-icon">✓</span>
              <span>All tests passed! Ready to ship.</span>
            </div>
          `}
        </div>

        <div class="kn-test-items">
          ${testItems.map(function (item) {
      ...
      return `
              <div class="kn-test-item">
                ...
                <button 
                  type="button" 
                  class="kn-test-item__tooltip-btn" 
                  title="${item.tooltip}"
                  aria-label="How to test this item"
                >?</button>
              </div>
            `;
    }).join('')}
        ...
        <div class="kn-test-actions">
          <button type="button" class="kn-button kn-button--secondary" id="reset-test-status">
            Reset Test Status
          </button>
        </div>
      </section>
    ...
    const checkboxes = container.querySelectorAll('.kn-test-item__checkbox');
    checkboxes.forEach(function (checkbox) {
      checkbox.addEventListener('change', function () {
        const testKey = this.getAttribute('data-test-key');
        toggleTestItem(testKey);
        renderTest(container);
      });
    });

    const resetBtn = container.querySelector('#reset-test-status');
    if (resetBtn) {
    ...
    const tooltipBtns = container.querySelectorAll('.kn-test-item__tooltip-btn');
  ...
  function renderShip(container) {
    ...
    if (!allPassed) {
      container.innerHTML = `
        <header class="kn-route__header">
          <h1 class="kn-heading-1">Ship</h1>
          <p class="kn-route__subtext">
            This page is locked until all tests pass.
          </p>
        </header>
        
        <section class="kn-ship-locked">
          <div class="kn-alert kn-alert--error">
            <h2 class="kn-alert__title">🔒 Ship Locked</h2>
            <p class="kn-alert__body">
              You must complete all test checklist items before accessing this page.
              <br><br>
              <a href="/jt/07-test" class="kn-button kn-button--primary" data-route-key="test">
      ...
      container.innerHTML = `
        ...
        <section class="kn-ship-ready">
          <div class="kn-alert kn-alert--success">
            <h2 class="kn-alert__title">✓ Ready to Ship</h2>
            <p class="kn-alert__body">
              All test checklist items have been verified. The Job Notification Tracker is ready for production.
            </p>
          </div>

          <div class="kn-ship-content">
            <div class="kn-card">
              <div class="kn-card__header">
                <h2 class="kn-heading-3">Deployment Checklist</h2>
                <p class="kn-card__subtitle">
                  Final steps before going live.
                </p>
              </div>
              <div class="kn-card__body">
                <ul class="kn-ship-list">
                  <li>Review all test results one final time</li>
                  <li>Verify data persistence across sessions</li>
                  <li>Check responsive design on different screen sizes</li>
                  <li>Confirm all features match requirements</li>
                  <li>Document any known limitations</li>
                </ul>
              </div>
            </div>
          </div>
  ...
  function updateTopbarStatus() {
    const statusBadge = document.querySelector('.kn-topbar__section--right .kn-badge--status');
    if (!statusBadge) return;

    const status = getProjectStatus();
    const statusClass = status === 'Shipped' ? 'kn-badge--status-shipped' :
      (status === 'In Progress' ? 'kn-badge--status-in-progress' : 'kn-badge--status-not-started');

    // Remove old status classes
    statusBadge.classList.remove('kn-badge--status-shipped', 'kn-badge--status-in-progress', 'kn-badge--status-not-started');
  ...
  window.addEventListener("DOMContentLoaded", function () {
    ...
    // Initial route render based on current location (check hash for file:// URLs)
    const initialPath = window.location.hash ? window.location.hash.substring(1) : (window.location.pathname || "/");
    navigateTo(initialPath, { skipHistory: true });
    ...
    // Handle hash changes (for file:// URLs)
